...
from StormBeatz import YouTube, app
from StormBeatz.core.call import StormBeatz
from StormBeatz.misc import SUDOERS, db
from StormBeatz.utils.database import (is_active_chat,
...
from StormBeatz.utils.formatters import seconds_to_min
from StormBeatz.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from StormBeatz.utils.stream.autoclear import auto_clean
from StormBeatz.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await StormBeatz.pause_stream(chat_id)
        ...
        await StormBeatz.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await StormBeatz.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await StormBeatz.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await StormBeatz.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await StormBeatz.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
